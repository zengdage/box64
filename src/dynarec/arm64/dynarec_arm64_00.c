            ...
            if (IsBridgeSignature(PK(0), PK(1))) {
                ...
                if(PK64(0)==0)
                {
                    ...
                    *need_epilog = 1;
                    ...
                    MESSAGE(LOG_DUMP, "Native Call to %s\n", GetBridgeName(native_name, (void*)ip) ?: GetNativeName(native_name, GetNativeFnc(ip), 1));
            ...
            switch(tmp) {
                case 3:
                    ...
                    if ((BOX64ENV(log) < 2 && !BOX64ENV(rolling_log) && !BOX64ENV(dynarec_test)) && dyn->insts[ninst].natcall) {
                        ...
                        tmp=0;
                    MESSAGE(LOG_DUMP, "Native Call to %s (retn=%d, simpleWrapper=%d)\n", GetBridgeName(native_name, (void*)(dyn->insts[ninst].natcall - 1)) ?: GetNativeName(native_name, GetNativeFnc(dyn->insts[ninst].natcall - 1), 1), dyn->insts[ninst].retn, tmp);
